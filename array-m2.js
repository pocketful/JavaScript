...
const studentsFromVln = students.filter((studObj) => studObj.town === 'Vilnius');
...
const studentsFemale = students.filter((studObj) => studObj.gender === 'female');
...
const everyAgeIsNumber = students.every((studObj) => typeof studObj.age === 'number');
...
const onlyNameAndCity = students.map((studObj) => ({ name: studObj.name, town: studObj.town }));
...
const totalStudentsAge = students.reduce((total, studObj) => total + studObj.age, 0);
console.log('totalStudentsAge ===', totalStudentsAge);


// 9. Suskaiciuoti visu zmoniu metu vidurki
const totalStudentsAgeAverage = students.reduce((total, studObj) => total + studObj.age / students.length, 0);
