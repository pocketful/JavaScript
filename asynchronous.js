...
myFirstPromise
  .then((successMsg) => {
    console.log(successMsg); // Promise is resolved!
  })
  .catch((errorMsg) => {
...
// The word “async” before a function means that a function always returns a promise. Other values are wrapped in a resolved promise automatically.
// For instance, this function returns a resolved promise with the result of 1:
async function f() {
  return 1;
}
f().then(alert); // Promise {<fulfilled>: 1}
