...
const maxNumber2 = numbers.reduce((total, item) =>
...
function olderThanAgeStudents(array, year) {
  const studentsOlderThanAge = array.filter(
    (studentObj) => studentObj.age > year
  );
...
// 2 way
function generateNameList2(array) {
  const ulEl = document.createElement("ul");
  // map way
  ulEl.innerHTML = array
    .map((arrayObj) => `<li> ${arrayObj.name} </li>`)
    .join("");
  // reduce way
  // ulEl.innerHTML = array.reduce((totalString, arrayObj) => totalString + `<li> ${arrayObj.name} </li>`, '');
  document.body.append(ulEl);
}
generateNameList2(students);
...
// 1 way
function generateNameList3(array) {
  const ulEl = document.createElement("ul");
  array.map((arrayObj) => {
    // map - forEach
    const liEl = document.createElement("li");
...
generateNameList3(students);

// 2 way
function generateNameList4(array) {
  const ulEl = document.createElement("ul");
  // map way
  ulEl.innerHTML = array
    .map(
      (arrayObj) =>
        `<li> ${arrayObj.name} <button class="del-stud">delete</button> </li>`
    )
    .join(""); // class tik teachers way
  // reduce way
  // ulEl.innerHTML = array.reduce((totalString, arrayObj) => totalString + `<li> ${arrayObj.name} </li>`, '');
  document.body.append(ulEl);
}
generateNameList4(students);
...
function generateCards(array) {
  ...
  cardsEl.innerHTML = array
    .map(
      (arrayObj) =>
        `<div class='card'><h3>${arrayObj.name}</h3><p>A ${arrayObj.gender} from ${arrayObj.town}.</p><p>${arrayObj.age} years old.</p><p> A car: ${arrayObj.hasCar}.</div>`
    )
    .join("");
...
function generateCityCards(array) {
  ...
  buttonKnsEl.addEventListener("click", (event) => {
    ...
    // cardsEl.classList.add('cards');
    cardsEl.innerHTML = array
      .filter((arrayObj) => arrayObj.town === "Kaunas")
      .map(
        (arrayObj) =>
          `<div class='card'><h3>${arrayObj.name}</h3><p>A ${arrayObj.gender} from ${arrayObj.town}.</p><p>${arrayObj.age} years old.</p><p> A car: ${arrayObj.hasCar}.</div>`
      )
...
kaunasBtn.addEventListener("click", () =>
  generateCards(studentsFrom("Kaunas"))
);
vilniusBtn.addEventListener("click", () =>
  generateCards(students.filter((studObj) => studObj.town === "Vilnius"))
);
...
function isAvgWhole(arr) {
  const average = Number.isInteger(
    arr.reduce((total, num) => total + num) / arr.length
  );
  // 2 way
  const average2 = Number.isInteger(
    arr.reduce((total, num) => total + num / arr.length, 0)
  );
  // 3 way
...
function doubledLetters2(str) {
  ...
  // doubling only strings in array
  const doubledLettersArray = stringsArray.map((char) =>
...
function validatePostCode(postcode) {
  ...
  let result =
    regex3Dig2LettersSequence.test(postcode) ||
    regex5DigitsSequence.test(postcode);
  console.log(result);
}
...
let result2 = text2.match(pattern);
console.log(result2);

// // at least three digits:
// let num1 = /^(?:\D*\d){3}/
// str.match(/\d/g).length >= 3/\d.*\d.*\d/
