...
console.log(
  "negative numbers array with filter === ",
  negativeSkArrayWithFilter
);

/* 2 way, longer */
const negativeSkArrayWithFilter2 = skArray.filter((sk) => {
  return sk < 0;
});
console.log(
  "new negative numbers array with filter === ",
  negativeSkArrayWithFilter2
);

/* 3 way, longer  */
const negativeSkArrayWithFilter3 = skArray.filter((sk) => {
  if (sk < 0) {
    return true;
  }
});
console.log(
  "new negative numbers array with filter === ",
  negativeSkArrayWithFilter3
);

/* 4 way, longer  */
console.log("array before changing with filter ===", skArray);
const moreThanTenArray4 = skArray.filter(ifMoreThanTen);
console.log("moreThanTenArray filter ===", moreThanTenArray4);
...
// 2 way, longer
const newMultiplyArray2 = skArray.map(multiplyNum);
console.log("newMultiplyArray map ===", newMultiplyArray2);
...
const liStringFromArray2 = liArray.join("");
ulEl.innerHTML = liStringFromArray2;

/* 2 way shorter */
const ulEl = document.getElementById("ul");
// gaunam masyva su li el tekstu
// const liArray = skArray.map(sk => `<li> skaicius ${sk} </li>`);
const liStringFromArray = skArray
