...
fetch(
  "https://randomuser.me/api/?seed=belekoks&inc=name,dob,email,picture&noinfo"
) // same user, less info
  .then((response) => response.json())
  .then((user) => {
    user = user.results[0];
...
fetch("https://boiling-reaches-93648.herokuapp.com/week-3/wedding")
  ...
  .then((users) =>
    users.forEach((user) =>
      toTable(
        user.name,
        replaceToSign(user.plusOne),
        replaceToSign(user.attending)
      )
    )
  )
  .catch((error) => console.warn(error.message));
...
function replaceToSign(text) {
  ...
  return textStr === "true"
    ? textStr.replace("true", "+")
    : textStr.replace("false", "-");
}
