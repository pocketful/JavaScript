...
function getCookie(cname) {
  ...
  for (let i = 0; i < cookieArray.length; i++) {
    let c = cookieArray[i];
    while (c.charAt(0) == " ") {
      // ' wizard=Hermione Granger' // nes jei tarpas pradzioj
      c = c.substring(1); // nukerpam po pirmo char, lieka 'wizard=Hermione Granger'
    }
    if (c.indexOf(name) == 0) {
      // jeigu prasideda wizard. indexOf() returns the position of the first occurrence of a value in a string
...
document.cookie =
  "showAlert=true; expires=" + cookieTimeDateFormat.toUTCString();
